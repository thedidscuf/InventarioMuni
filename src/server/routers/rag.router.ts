  ...
  generateText: Trpc.procedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const ragVector = await RagService.getVectorStore(input.tags)

        // If the RAG vector is not found, fallback to OpenAI
        if (!ragVector && OpenaiService.isActive()) {
          console.log('RAG vector not found, using OpenAI')
          const context =
            'No relevant documents were found in our document repository, so generating a general response.'

          const generateTextOptions = { prompt, context, history }
          const answer = await OpenaiService.generateText(generateTextOptions)
          return { answer }
        }
