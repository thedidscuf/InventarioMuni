...
class Service {
   ...
   */
  async query(prompt: string, vectorData: VectorData, history: string[] = []) {
    const { vectorStore, pathStorage } = vectorData
  ...
  async getVectorStore(tags: string[]): Promise<VectorData> {
    let vectorStore: HNSWLib
    let pathStorage: string
    ...
    if (isTemporary) {
      ...
      try {
        ...
        pathStorage = result.pathStorage
      } catch (error) {
        return null
      }
    ...
    return { vectorStore, pathStorage }
  }
